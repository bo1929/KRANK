  ...
  , _specified_batch(specified_batch)
{
  ...
  if (!_in_library) {
    ...
    if (_specified_batch != 0) {
      std::cerr
        << "A specific batch can not be built before saving processed k-mer sets into library."
        << std::endl;
      exit(EXIT_FAILURE);
    }
  } else {
    ...
    std::puts(
      "Given parameters will be ignored. Metadata of the given library in the disk is loaded.\n");
  }
...
Library::run(uint8_t sdepth, SelectionMethod init_selection)
{
  if (_switch_selection && init_selection == random_kmer)
    std::puts("No counterpart to current selection method, no switching and only random.\n");
