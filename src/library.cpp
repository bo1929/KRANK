...
Library::Library(const char *library_dirpath, const char *nodes_filepath,
                 const char *input_filepath, uint8_t k, uint8_t h, uint8_t b,
                 uint64_t capacity_size, uint32_t tbatch_size,
                 bool swicth_selection, bool in_library, bool on_disk,
                 uint8_t specified_batch)
    : _library_dirpath(library_dirpath), _taxonomy_ncbi(nodes_filepath),
      _taxonomy_record(input_filepath, _taxonomy_ncbi),
      _nodes_filepath(nodes_filepath), _input_filepath(input_filepath), _k(k),
      _h(h), _b(b), _capacity_size(capacity_size), _tbatch_size(tbatch_size),
      _switch_selection(swicth_selection), _in_library(in_library),
      _on_disk(on_disk), _specified_batch(specified_batch) {
  bool isDir = IO::ensureDirectory(_library_dirpath);
  if (!in_library) {
    if (isDir) {
      std::cout << "Library will be created at " << _library_dirpath
                << std::endl;
    } else {
      std::cerr << "Library can not be created at " << _library_dirpath
                << std::endl;
      exit(EXIT_FAILURE);
    }
  } else {
...
#pragma omp critical
    {
      std::cout << "Processing k-mer set for taxon "
...
#pragma omp critical
      {
        _streamOD_map.emplace(
            std::make_pair(tID_key, StreamOD<encT>(disk_path.c_str())));
...
#pragma omp critical
  ...
  std::cout << "Total number of (non-unique) k-mers under the root: "
            << _root_size << std::endl;
  bool taxonomy_saved = _taxonomy_record.saveTaxonomyRecord(_library_dirpath);

  std::cout
      << "Library is initialized and k-mers sets are ready to be streamed."
      << std::endl;
  if (on_disk)
  ...
  std::cout << "\tNumber of positions of LSH, h: " << std::to_string(_h)
            << std::endl;
  std::cout << "\tNumber of columns in the table, b: " << std::to_string(_b)
            << std::endl;
  ...
  std::cout << "\tTotal number of (non-unique) kmers: " << _root_size
            << std::endl;
  std::cout << std::endl;
...
void Library::run(uint8_t sdepth, SelectionMethod init_selection) {
  ...
  for (unsigned int i = 0; i < _total_batches; ++i) {
    ...
    if ((_specified_batch == 0) || (_specified_batch == curr_batch)) {
      std::cout << "Building the library for batch " << curr_batch << "..."
                << std::endl;
...
uint64_t Library::getConstrainedSize(std::set<tT> tIDsBasis) {
  ...
  float sq_ratio =
      sqrt(static_cast<float>(sum_size) / static_cast<float>(_root_size));
  float batch_ratio =
      static_cast<float>(_tbatch_size) / static_cast<float>(_num_rows);
  constrained_size =
      static_cast<uint64_t>(_capacity_size * sq_ratio * batch_ratio);
  return constrained_size;
}

void Library::getBatchHTs(HTs<encT> *ts, uint8_t curr_depth,
                          uint8_t last_depth) {
  if ((curr_depth >= last_depth) || _taxonomy_record.isBasis(ts->tID)) {
    HTd<encT> td(ts->tID, ts->k, ts->h, ts->num_rows, ts->ptr_lsh_vg,
                 ts->kmer_selection);
    getBatchHTd(&td);
    td.convertHTs(ts);
  } else {
    ...
    ts->childrenHT.assign(num_child,
                          HTs<encT>(0, ts->k, ts->h, ts->b, ts->num_rows,
                                    ts->ptr_lsh_vg, next_selection));
    ...
    ts->childrenHT.clear();
    std::cout << "HTs has been constructed for "
...
void Library::getBatchHTd(HTd<encT> *td) {
  if (_taxonomy_record.isBasis(td->tID)) {
    ...
    td->initBasis(td->tID);
    td->makeUnique();
    std::cout << "Basis HTd has been constructed for "
    ...
    td->childrenHT.assign(num_child,
                          HTd<encT>(0, td->k, td->h, td->num_rows,
                                    td->ptr_lsh_vg, td->kmer_selection));
    for (unsigned int ti = 0; ti < num_child; ++ti) {
    ...
    td->childrenHT.clear();
    std::cout << "HTd has been constructed for "
              << _taxonomy_record.changeIDtax(td->tID) << std::endl;
  }
}
...
bool Library::saveBatchHTs(HTs<encT> &ts, uint16_t curr_batch) {
  ...
  {
    FILE *encf = IO::open_file(
        (save_dirpath + "/enc_arr-" + std::to_string(curr_batch)).c_str(),
        is_ok, "wb");
  ...
  {
    FILE *indf = IO::open_file(
        (save_dirpath + "/ind_arr-" + std::to_string(curr_batch)).c_str(),
        is_ok, "wb");
...

  {
    FILE *tlcaf = IO::open_file(
        (save_dirpath + "/tlca_arr-" + std::to_string(curr_batch)).c_str(),
        is_ok, "wb");
...

  {
    FILE *scountf = IO::open_file(
        (save_dirpath + "/scount_arr-" + std::to_string(curr_batch)).c_str(),
        is_ok, "wb");
...
bool Library::loadMetadata() {
  ...
  FILE *metadataf =
      IO::open_file((save_dirpath + "/metadata").c_str(), is_ok, "rb");
  ...
  std::fread(bases_sizes.data(), sizeof(std::pair<tT, uint16_t>), _num_species,
             metadataf);
  _positions.resize(_h);
  std::fread(_positions.data(), sizeof(uint8_t), _positions.size(), metadataf);
  _npositions.resize(_k - _h);
  std::fread(_npositions.data(), sizeof(uint8_t), _npositions.size(),
             metadataf);
...
bool Library::saveMetadata() {
  bool is_ok = true;
  std::string save_dirpath(_library_dirpath);
  std::vector<std::pair<tT, uint16_t>> bases_sizes(_basis_to_size.begin(),
                                                   _basis_to_size.end());

  FILE *metadataf =
      IO::open_file((save_dirpath + "/metadata").c_str(), is_ok, "wb");
  ...
  std::fwrite(bases_sizes.data(), sizeof(std::pair<tT, uint16_t>),
              bases_sizes.size(), metadataf);
  std::fwrite(_positions.data(), sizeof(uint8_t), _positions.size(), metadataf);
  std::fwrite(_npositions.data(), sizeof(uint8_t), _npositions.size(),
              metadataf);
