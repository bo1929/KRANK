...
namespace AixLog {
  ...
  struct SinkFile : public SinkFormat
  {
    ...
    void log(const Metadata &metadata, const std::string &message) override { do_log(ofs, metadata, message); }
  ...
  struct SinkEventLog : public Sink
  {
    ...
    {
  #ifdef UNICODE
      std::wstring wide = std::wstring(ident.begin(), ident.end()); // stijnvdb: RegisterEventSource expands to
