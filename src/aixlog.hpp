...
#define AIXLOG_INTERNAL__LOG_MACRO_CHOOSER(...)                                                    \
  AIXLOG_INTERNAL__VAR_PARM(                                                                       \
    __VA_ARGS__, AIXLOG_INTERNAL__LOG_SEVERITY_TAG, AIXLOG_INTERNAL__LOG_SEVERITY, )
...
// stijnvdb: sorry! :) LOG(SEV, "tag") was not working for Windows and I couldn't figure out how to
// fix it for windows without potentially breaking everything else...
...
enum class Severity : std::int8_t
{
  ...
  // notice                  NOTICE      INFO        INFO        SUCCESS       normal, but
  // significant, condition warning     warning     WARNING     WARN        DEFAULT     WARNING
  // warning conditions error       error       ERROR       ERROR       ERROR       ERROR error
  // conditions fatal       fatal       CRIT        FATAL       FAULT       ERROR         critical
  // conditions
  //                         ALERT                                             action must be taken
  //                         immediately EMERG                                             system is
  //                         unusable
...
static Severity
to_severity(std::string severity, Severity def = Severity::info)
{
...
static std::string
to_string(Severity logSeverity)
{
  switch (logSeverity) {
    case Severity::trace:
      return "Trace";
...
struct Conditional
{
  ...
  Conditional()
    : func_([](void) { return true; })
  {}

  Conditional(const EvalFunc& func)
    : func_(func)
  {}

  Conditional(bool value)
    : func_([value](void) { return value; })
  {}
...
struct Timestamp
{
  ...
  std::string to_string(const std::string& format = "%Y-%m-%d %H-%M-%S.#ms") const
  {
    ...
    if (pos != std::string::npos) {
      int ms_part = std::chrono::time_point_cast<std::chrono::milliseconds>(time_point)
                      .time_since_epoch()
                      .count() %
                    1000;
      char ms_str[4];
...
static std::ostream&
operator<<(std::ostream& os, const Severity& log_severity);
static std::ostream&
operator<<(std::ostream& os, const Timestamp& timestamp);
static std::ostream&
operator<<(std::ostream& os, const Tag& tag);
static std::ostream&
operator<<(std::ostream& os, const Function& function);
static std::ostream&
operator<<(std::ostream& os, const Conditional& conditional);
static std::ostream&
operator<<(std::ostream& os, const Color& color);
static std::ostream&
operator<<(std::ostream& os, const TextColor& text_color);
...
protected:
  ...
  {
    std::clog.rdbuf(this);
    std::clog << Severity() << Tag() << Function() << Conditional() << AixLog::Color::NONE
              << std::flush;
  }
...
protected:
  virtual void do_log(std::ostream& stream,
...
{
  SinkOutputDebugString(const Filter& filter)
    : Sink(filter)
  {}
...
{
  SinkUnifiedLogging(const Filter& filter)
    : Sink(filter)
  {}
  ...
  {
    os_log_with_type(
      OS_LOG_DEFAULT, get_os_log_type(metadata.severity), "%{public}s", message.c_str());
  }
};
#endif
...
{
  SinkSyslog(const char* ident, const Filter& filter)
    : Sink(filter)
  {
...
struct SinkAndroid : public Sink
{
  ...
  {
    std::string tag =
      metadata.tag ? metadata.tag.text : (metadata.function ? metadata.function.name : "");
    std::string log_tag;
...
{
  SinkEventLog(const std::string& ident, const Filter& filter)
    : Sink(filter)
  {
  #ifdef UNICODE
    std::wstring wide =
      std::wstring(ident.begin(), ident.end()); // stijnvdb: RegisterEventSource expands to
 ...
 *
 * Severity must be the first thing that is logged into clog, since it will reset the loggers
 * metadata.
 */
static std::ostream&
operator<<(std::ostream& os, const Severity& log_severity)
