...
int main(int argc, char **argv)
{
  ...
  sub_build->add_flag("--from-library,!--from-scratch",
                      ...
                      "Default: --from-scratch, and it reads k-mer sets or "
                      "sequences from given input paths.");
  ...
  sub_build->add_option("--target-batch",
                        ...
                        "If not given, library will only be initialized after "
                        "reading the input data and encoding k-mers.");
  sub_build->callback([&]() {
  ...
  sub_build
    ->add_option("--kmer-ranking",
                 ranking_method,
                 "Which strategy will be used for k-mer ranking? "
                 "(random_kmer, representative_kmer)")
  ...
  sub_query->add_option("--max-match-distance,--max-match-hdist",
                        ...
                        "considered as a match. Default: 5.");
  ...
  if (b < 2)
    std::cerr << "The number of columns of the hash table, b, can not be "
                 "smaller than 2."
              << std::endl;
  if (h < 2)
    std::cerr << "The number of positions for LSH, h, can not be smaller than 2." << std::endl;
  if (h >= k)
    std::cerr << "The number of positions for LSH, h, can not be greater than "
                 "or equal to k-mer length."
              << std::endl;
  if (batch_size > (2 * h - 1))
    std::cerr << "The number of bits to determine the number of batches must "
                 "be smaller than 2h."
              << std::endl;
  if (target_batch > pow(2, batch_size))
    std::cerr << "The given target batch index (starts from 1) is greater than "
                 "the number of batches."
              << std::endl;
  if (!(on_disk || !from_library))
    std::cerr << "If k-mer sets are going to be read from the library, "
                 "--on-disk must be set."
              << std::endl;
  if (!((target_batch != 0) || !from_library))
    std::cerr << "If a target batch is specified (other than 0), k-mer sets "
                 "must be read from the library."
              << std::endl;
  if (max_match_hdist > k)
    std::cerr << "Maximum Hamming distance for a match can not be greater than "
                 "k-mer length."
              << std::endl;
...
#ifndef SHORT_TABLE
  if ((k - h) > 16) {
    std::cerr << "In order to use compact k-mer encodings, k and h must "
                 "satisfy (k-h) <= 16."
              << std::endl;
    exit(EXIT_FAILURE);
  }
