...
main(int argc, char** argv)
{
  ...
  sub_build->add_option(
    "--switch-depth",
    switch_depth,
    "The depth in the taxonomic tree at which the k-mer selection strategy will be switched.");
  ...
  sub_build->add_option(
    "--in-library", in_library, "Are k-mers already processed and is in the library?");
  bool on_disk = true;
  sub_build->add_option("--on-disk",
                        on_disk,
                        "Are stream of k-mers will be on the disk? If so, k-mers will be read in "
                        "batches from the disk.");
  std::map<std::string, SelectionMethod> map{ { "random_kmer", random_kmer },
                                              { "large_scount", large_scount },
                                              { "information_score", information_score } };
  SelectionMethod kmer_selection = random_kmer;
  sub_build
    ->add_option("--kmer-selection",
                 kmer_selection,
                 "Which strategy will be used for k-mer selection? (random_kmer, large_scount, "
                 "information_score)")
  ...
  bool run_build = true;
  sub_build->add_option(
    "--run", run_build, "Run library building in addition to initialization. Default is true.");
  ...
  sub_query->add_option(
    "--save-match-info",
    save_match_info,
    "Save macthing information to --output-dir for each query. Default is true.");
  sub_query->add_option(
    "--num-threads", num_threads, "Number of threads to use for OpenMP based parallelism.");
  bool run_query = true;
  sub_query->add_option(
    "--run", run_query, "Run queries in addition to instance initialization. Default is true.");

  CLI11_PARSE(app, argc, argv);

  bool switch_selection = false;
  if (switch_depth > 0)
  ...
  if (sub_build->parsed()) {
    Library lib(library_dir.c_str(),
                ...
                specify_batch);
    if (run_build)
      lib.run(switch_depth, kmer_selection);
  }
