...
int main(int argc, char **argv) {
  ...
  bool log = false;
  app.add_flag("--log,!--no-log", log,
               "Increased verbosity and extensive logging.");
  app.require_subcommand();

  CLI::App *sub_build = app.add_subcommand(
      "build", "Builds a referenece library with given k-mers sets.");
  ...
  sub_build
      ->add_option("-t,--taxonomy-dmp", taxonomy_dmp,
                   "Path to the file containing the taxonomy dmp.")
      ->required();
  ...
  sub_build->add_option("-k,--kmer-length", k,
                        "Length of k-mers. Default is 32.");
  uint8_t h = 11;
  sub_build->add_option("-h,--num-positions", h,
                        "Number of positions for the LSH. Default is 14.");
  uint8_t b = 8;
  sub_build->add_option("-b,--num-columns", b,
                        "Number of columns of the table. Default is 8.");
  uint8_t switch_depth = 4;
  sub_build->add_option(
      "--switch-depth", switch_depth,
  ...
  std::map<std::string, RankingMethod> map_ranking{
      {"random_kmer", random_kmer},
      {"species_count", species_count},
      {"information_score", information_score},
      {"taxa_count", taxa_count}};
  std::pair<RankingMethod, RankingMethod> ranking_methods = {random_kmer,
                                                             random_kmer};
  sub_build
      ->add_option(
          "--kmer-ranking", ranking_methods,
  ...
  sub_query
      ->add_option("-o,--output_dir", output_dir,
  ...
  sub_query->add_flag("--save-match-info,!--no-match-info", save_match_info,
  ...
  bool run_query = true;
  sub_query->add_flag(
      "--run,!--only-init", run_query,
      "Run queries in addition to instance initialization, given by default.");

  CLI11_PARSE(app, argc, argv);

  if (sub_build->parsed()) {
    Library lib(library_dir.c_str(), taxonomy_dmp.c_str(), input_file.c_str(),
                k, h, b, ranking_methods.first, ranking_methods.second,
                pow(2, 2 * h) * b,          // capacity
                pow(2, 2 * h - batch_size), // tbatch_size
                in_library, on_disk, specify_batch, log);
    if (run_build)
      lib.run(switch_depth);
  }

  if (sub_query->parsed()) {
    Query query(library_dir_v, output_dir.c_str(), query_file.c_str(),
                max_match_hdist, save_match_info, log);
    if (run_query)
      query.run();
  }
