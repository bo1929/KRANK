...
Running `krank --help` will display available subprograms together brief descriptions regarding them and main arguments.
...
The path to directory which the krank library that will be initialized, built or updated must be stated with `--library-dir` or `-l`.
...
If each line (2 lines actually, if FASTA) is a sequence (such as a scaffold) in the input data, then use `--input-sequences` flag.
Otherwise, if each line corresponds to a *k*-mer extracted with a external tool, then use `--input-kmers` flag (default).
...
#### Taxonomy
KRANK only expects a taxonomy tree, as a nodes file `nodes.dmp`, which you can download from NCBI website.
...
#### Library parameters
There are two parameters that defines the shape of the final hash table: `-h` and `-b` (`--num-positions` and `--num-columns`, respectively).
...
In order to keep memory usage feasible, there are two ways: use `--on-disk` flag (default) and increase the number of batches using `--batch-size`.
...
When the final library is built, you can delete them (files with prefix `lsh_enc_vec-*`).
...
If you have cluster with many nodes, you can process each of these batches in parallel on different nodes, using multiple cores too (by submitting each batch's job to a different node).
...
You can configure this behavior using `--target-batch` and `--from-library` options as showed below.
...
./krank build \
  ...
  --num-threads 32
```
This will initialize the library, and save the metadata & taxonomy file together with encoding and LSH keys (since `--on-disk` given).

Then you can either process 8 batches one by one using 32 cores as follows:
```bash
./krank build \
  --library-dir /path/to/directory --taxonomy-dmp /path/to/nodes.dmp --input-files /path/to/mapping.tsv \
  -k 28 -w 31 -h 12 -b 16 --batch-size 3 \
