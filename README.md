...
Querying against a file generates two output files by default: a taxonomic classification for each sequence (short/long read or contig) and a summary of the taxonomic composition of the entire query file, i.e., relative abundances of taxa at different levels.

The classification file is named as prefix + query ID (or prefix + filename without the extension if a single fine is given): `classification_info-*`. The first couple of lines should look like the following example.
```
...
```
`READ_COUNT` columns are simply the counts of query sequences that are assigned to a particular taxon.
...
Otherwise (initialization with `--from-scratch`, default), k-mers would be processed again, and LSH keys would be overwritten, possibly computed with a new random seed.
Hence, the `--from-library` option must be given to build the initialized library.
